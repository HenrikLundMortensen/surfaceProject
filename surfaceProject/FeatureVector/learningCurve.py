...
def clusterNumMatFromKmeans(Xtest,kmeans):
    ...
    # Ftest describes each surface with Na x Nf numbers (Nf features for each atom)
    Ftest = fv.getBondFeatureVectors(Xtest)
    (Ntest,Na,Nf) = np.shape(Ftest)
    
    # Reduces the Nf features for each atom into one number describing which cluster it
